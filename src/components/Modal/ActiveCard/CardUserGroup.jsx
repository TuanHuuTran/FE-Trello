...
function CardUserGroup( { cardMemberIds = [], onUpdateCardMembers } ) {
  ...
  // Lấy board ra vì có trường FE_allUsers:
  const board = useSelector( selectCurrentActiveBoard )

  // lấy được trường memberIds thông qua FE_allUsers
  //const FE_MemberIds = board.FE_allUsers.filter( user => cardMemberIds.includes( user._id ) )

  const FE_MemberIds = cardMemberIds.map( id => {
    return board.FE_allUsers.find( u => u._id === id )
  } )

  const handleUpdateCardMembers = ( user ) => {
    // Tạo một biến inComingMemberInfo để gửi cho BE, với 2 thông tin chính là userId và action là xóa (REMOVE) hay thêm (ADD)vào card
